...
export default function NarrativePanel({
  ...
  // On a new message, scroll its TOP edge into view rather than jumping to
  // the absolute bottom of the log -- a long DM reply otherwise lands with
  // its opening scrolled off-screen above, forcing a scroll back up just to
  // read it from the start. Only does this if the reader was already near
  // the bottom; if they've scrolled up into earlier history, a new message
  // shouldn't yank them away from it -- show a "Jump to latest" nudge instead.
  useEffect(() => {
    ...
    <div className="flex flex-col h-full relative">
      <div ref={scrollRef} onScroll={onScroll} className="flex-1 overflow-y-auto px-3 md:px-6 py-4 space-y-4">
        {summary && !recapDismissed && (
          <div className="panel rounded-lg p-3 max-w-2xl">
            <div className="flex items-center justify-between mb-1">
              <span className="text-lg uppercase tracking-widest text-gold/70">Story So Far</span>
              <button
                onClick={() => setRecapDismissed(true)}
                aria-label="Dismiss story so far"
                className="text-parchment/40 hover:text-parchment/80 text-lg"
              >
                ✕
              </button>
            </div>
            <p className="text-lg text-parchment/70 line-clamp-4 whitespace-pre-wrap">{summary}</p>
          </div>
        )}
        {entries.length === 0 && (
        ...
        {entries.map((entry, idx) => (
          <div
            ...
            className={entry.role === "player" ? "text-right" : "text-left"}
          >
            <div
              className={`inline-block max-w-[90%] md:max-w-2xl text-left rounded-lg px-3 md:px-4 py-2 whitespace-pre-wrap leading-relaxed text-xl md:text-2xl ${
                entry.role === "player"
        ...
        {isLoading && (
          <div className="text-left">
            <div className="inline-flex items-center gap-2 rounded-lg px-4 py-2 bg-ink-800/50 text-parchment/50 italic text-xl">
        ...
        {error && (
          <div className="text-left">
            <div className="inline-block rounded-lg px-4 py-2 bg-blood-dark/40 border border-blood-light/40 text-parchment text-xl">
              {error}
            </div>
          </div>
        )}
        {defeatOccurred && (
          <div className="text-center py-2">
            <div className="inline-block px-4 py-2 rounded-lg border border-blood/60 bg-blood/20 shadow-glow-blood">
              <p className="text-lg uppercase tracking-widest text-parchment/80">You were defeated — but you survived</p>
            </div>
          </div>
        )}
      </div>
      ...
      {showJumpToLatest && (
        <button
          ...
          className="absolute bottom-24 left-1/2 -translate-x-1/2 text-lg px-3 py-1.5 rounded-full bg-ink-800 border border-gold/40 text-parchment/80 shadow-glow-gold hover:text-parchment"
        >
          ↓ Jump to latest
        </button>
      )}
        ...
        <div className="px-3 md:px-6 pb-2 flex gap-2 overflow-x-auto">
          {quickActions.map((qa) => (
            <button
              ...
              onClick={() => submit(qa)}
              disabled={isLoading}
              className="shrink-0 whitespace-nowrap text-lg px-3 py-1.5 rounded-full border border-gold/25 text-parchment/70 hover:text-parchment hover:border-gold/50 disabled:opacity-40"
            >
              {qa}
            </button>
          ))}
      ...
      <div className="border-t border-gold/15 p-3 md:p-4 pb-safe flex gap-2">
        ...
        <button
          ...
          className="min-h-[44px] px-4 py-2 rounded-md bg-blood hover:bg-blood-light disabled:opacity-40 disabled:cursor-not-allowed text-parchment text-xl font-medium self-end shadow-glow-blood"
        >
          Send
        </button>
      </div>
