    ...
    <div className="p-6 h-full overflow-y-auto">
      ...
      {linkedTo && (
        <div className="mb-4 rounded-lg border border-blood/40 bg-blood/10 p-3">
          ...
          {related.length === 0 ? (
            <p className="text-xs text-parchment/50">No other facts share a tag with this one.</p>
          ) : (
                ...
                {tagList(f.tags).length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {tagList(f.tags).map((t, i) => (
                      <span key={i} className="text-[10px] px-1.5 py-0.5 bg-white/5 rounded text-parchment/40">
                        {t}
                      </span>
                    ))}
                  </div>
      ...
      {filtered.length === 0 && <p className="text-sm text-parchment/40">No facts match &ldquo;{query}&rdquo;.</p>}
    </div>
  );
}
